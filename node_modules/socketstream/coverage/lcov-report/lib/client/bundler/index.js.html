...
<html lang="en">
<head>
    <title>Code coverage report for lib/client/bundler/index.js</title>
    <meta charset="utf-8">
...
<div class="header high">
    <h1>Code coverage report for <span class="entity">lib/client/bundler/index.js</span></h1>
    <h2>
        Statements: <span class="metric">95.98% <small>(239 / 249)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Branches: <span class="metric">83.06% <small>(103 / 124)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Functions: <span class="metric">94.64% <small>(53 / 56)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Lines: <span class="metric">96.3% <small>(234 / 243)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Ignored: <span class="metric"><span class="ignore-none">none</span></span> &nbsp;&nbsp;&nbsp;&nbsp;
    </h2>
    <div class="path"><a href="../../../index.html">All files</a> &#187; <a href="index.html">lib/client/bundler/</a> &#187; index.js</div>
</div>
<div class="body">
...
<span class="cline-any cline-yes">11</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">// Client-Side Bundler of assets in development and production
'use strict';
&nbsp;
var fs = require('fs'),
    path = require('path'),
...
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (client.bundler) { <span class="cstat-no" title="statement not covered" >return client.bundler; </span>}
&nbsp;
  if (client.ts) {
...
&nbsp;
  function systemModule(name,wrap) {
    ...
    if (mod) {
      var code = wrap===false? <span class="branch-0 cbranch-no" title="branch not covered" >mod.content:</span> ss.bundler.wrapModule(name, mod.content);
      return {
        file: mod.name,
        name: mod.name,
        path: mod.path,
...
&nbsp;
    create: function create(bundler) {
      ...
      <span class="missing-if-branch" title="else path not taken" >E</span>if (bundler) {
...
&nbsp;
      if (typeof pathsOrFunc === "function") {
        ...
        bundler.dests = bundler.define(args[2], args[3], args[4], args[5]);
      } else {
...
&nbsp;
          ...
          <span class="missing-if-branch" title="else path not taken" >E</span>if (index &amp;&amp; !firstIndex) {
...
&nbsp;
    load: function() {
      for(var n in bundlers) {
        ...
        <span class="missing-if-branch" title="else path not taken" >E</span>if (bundlers[n].load) {
            bundlers[n].load();
        }
      }
    },
&nbsp;
    unload: function() {
      for(var n in bundlers) {
        <span class="missing-if-branch" title="else path not taken" >E</span>if (bundlers[n].unload) {
          bundlers[n].unload();
...
&nbsp;
      ...
      <span class="missing-if-branch" title="else path not taken" >E</span>if (!(options.packedAssets &amp;&amp; options.packedAssets.keepOldFiles)) {
...
&nbsp;
    loadFile: function loadFile(entry, opts, formatter, cb, errCb) {
      ...
      formatter = formatter || ss.client.formatters[entry.ext || <span class="branch-1 cbranch-no" title="branch not covered" >type]</span>;
      <span class="missing-if-branch" title="if path not taken" >I</span>if (!formatter) {
<span class="cstat-no" title="statement not covered" >        throw new Error('Unsupported file extension \'.' + entry.ext + '\' when we were expecting some type of ' +</span>
          ((type||'unknown').toUpperCase()) + ' file. Please provide a formatter for ' + (entry.file) + ' or move it to /client/static');
      }
      <span class="missing-if-branch" title="if path not taken" >I</span>if (formatter.assetType !== type) {
<span class="cstat-no" title="statement not covered" >        throw new Error('Unable to render \'' + entry.file + '\' as this appears to be a ' + (type.toUpperCase()) +</span>
...
&nbsp;
    minifyJS: function minifyJS_(files) {
      var min = files.map(function(js) {
        return js.options.minified ? <span class="branch-0 cbranch-no" title="branch not covered" >js.content </span>: minifyJS(js.content);
      });
      return min.join('\n');
    },
&nbsp;
...
&nbsp;
      ...
      result.ext = extension? extension.substring(1) : (<span class="branch-1 cbranch-no" title="branch not covered" >result.assetType || result.bundle)</span>;
&nbsp;
      return result;
    },
&nbsp;
...
&nbsp;
    systemModules: <span class="fstat-no" title="function not covered" >function(wrap) {</span>
<span class="cstat-no" title="statement not covered" >      return Object.keys(system.assets.modules).map(<span class="fstat-no" title="function not covered" >function(name) {</span></span>
<span class="cstat-no" title="statement not covered" >        return systemModule(name,wrap);</span>
      });
    },
&nbsp;
...
&nbsp;
      <span class="missing-if-branch" title="if path not taken" >I</span>if (typeof options.entryModuleName === 'string' || options.entryModuleName === null) {
<span class="cstat-no" title="statement not covered" >        entryInit = options.entryModuleName? 'require("/'+options.entryModuleName+'");' : '';</span>
      }
&nbsp;
...
&nbsp;
      function isAssetType(entry) {
        <span class="missing-if-branch" title="if path not taken" >I</span>if (ss.client.formatters == null) {
...
<span class="cstat-no" title="statement not covered" >    return fs.mkdirSync(dir);</span>
  }
}
&nbsp;
...
<div class="footer">
    <div class="meta">Generated by <a href="http://istanbul-js.org/" target="_blank">istanbul</a> at Sun Jun 07 2015 10:39:18 GMT+0200 (CEST)</div>
</div>
