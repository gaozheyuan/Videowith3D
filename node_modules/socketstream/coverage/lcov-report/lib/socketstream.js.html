...
<html lang="en">
<head>
    <title>Code coverage report for lib/socketstream.js</title>
    <meta charset="utf-8">
...
<div class="header low">
    <h1>Code coverage report for <span class="entity">lib/socketstream.js</span></h1>
    <h2>
    ...
    <div class="path"><a href="../index.html">All files</a> &#187; <a href="index.html">lib/</a> &#187; socketstream.js</div>
</div>
<div class="body">
...
<span class="cline-any cline-no">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">// SocketStream 0.3
// ----------------
'use strict';
&nbsp;
// console.log('CHECK');
 ...
 */
<span class="fstat-no" title="function not covered" >function start(httpServer) {</span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >  api.publish = publish.api(server.eventTransport);</span>
&nbsp;
  // Start web stack
<span class="cstat-no" title="statement not covered" >  if (httpServer) {</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    api.log.info('Starting SocketStream %s in %s mode...'.green, version, env);</span>
&nbsp;
    // Bind responders to websocket
<span class="cstat-no" title="statement not covered" >    ws.load(httpServer, server.responders, server.eventTransport, session.options);</span>
&nbsp;
    // Append SocketStream middleware to stack
<span class="cstat-no" title="statement not covered" >    http.load(client.options.dirs['static'], client.options.dirs['assets'], server.sessionStore, session.options);</span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >    events.emit('server:start', server);</span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >      events.emit('server:stop', server);</span>
...
<span class="cstat-no" title="statement not covered" >    var sessionID = session.create();</span>
&nbsp;
    // jshint loopfunc:true
<span class="cstat-no" title="statement not covered" >    for (var id in server.responders) {</span>
<span class="cstat-no" title="statement not covered" >      if (server.responders.hasOwnProperty(id)) {</span>
<span class="cstat-no" title="statement not covered" >        var responder = server.responders[id];</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (responder.name &amp;&amp; responder.interfaces.internal) {</span>
...
<span class="cstat-no" title="statement not covered" >            var args = Array.prototype.slice.call(arguments),</span>
                cb = args.pop();
&nbsp;
<span class="cstat-no" title="statement not covered" >            return responder.interfaces.internal(args, {sessionId: sessionID, transport: 'test'}, <span class="fstat-no" title="function not covered" >function(err, params){</span> <span class="cstat-no" title="statement not covered" >cb(params); </span>});</span>
          };
<span class="cstat-no" title="statement not covered" >          api.add(responder.name, fn);</span>
        }
      }
    }
&nbsp;
...
exports.start = <span class="fstat-no" title="function not covered" >function(httpServer) {</span>
<span class="cstat-no" title="statement not covered" >  return serverInstance || (serverInstance = start(httpServer));</span>
};
&nbsp;</pre></td></tr>
</table></pre>
