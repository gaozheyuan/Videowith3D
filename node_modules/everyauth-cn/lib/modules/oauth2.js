  ...
  .getAccessToken( function (code, data) {
    ...
    request[this._accessTokenHttpMethod](opts, function (err, res, body) {
      ...
      if (parseInt(res.statusCode / 100) != 2) {
        ...
        return p.fail({extra: {res: res, data: body}});
      }
      var resType = res.headers['content-type'], data;
      ...
      // some sites(such as http://connect.qq.com) do not return `content-type` in response.
      data = querystring.parse(body);
      if(!data.access_token) data = JSON.parse(body);
